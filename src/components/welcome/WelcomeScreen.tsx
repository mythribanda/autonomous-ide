...
export const WelcomeScreen: React.FC = () => {
  const { setProject, openProjectWithDialog, openProject } = useProjectStore();
    ...
    try {
      ...
      if (path) {
        ...
        addToast({
          ...
          message: `Analyzed workspace intelligence for ${path}`
        });
      }
    } catch (err: any) {
      addToast({
        ...
        message: err?.message || 'Error opening project workspace'
      });
    }
  };
      ...
      <div className="max-w-3xl w-full space-y-6 py-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="px-4 py-2 rounded-sm bg-[#007ACC] hover:bg-[#0062A3] text-[#FFFFFF] font-bold text-xs flex items-center gap-2 transition-colors font-sans shadow-md"
          >
            <FolderOpen size={15} />
